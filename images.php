<?php
    include_once "management/modules/includes/class/Template.php";
  ...
  $Template= new Template();
	...
	while($row=@mysql_fetch_assoc($result)){
		...
		$Template->add_block('SECTION',array('NAME'=>$row['name']));
	}
	$per_page=19;
	...
	if($result){
	while($row=mysql_fetch_assoc($result)){
				$Template->add_block('PIC',array("ID"=>$row['id'],"NAME"=>$row['image_name'],"ADD"=>$row['image_add'],"SECTION"=>$row['image_section']));
  ...
  $Template->load_file('template/all_images.html');
